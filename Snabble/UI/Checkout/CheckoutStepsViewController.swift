...
public final class CheckoutStepsViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        super.viewDidLoad()
        updateHeaderView(headerView, with: checkoutProcess.paymentState)
    ...
    private func startTimer() {
        ...
        processTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: false) { [weak self] _ in
    ...
    private func stopTimer() {
        ...
        self.processSessionTask?.cancel()
        self.processSessionTask = nil
    }
    ...
    private func update(_ result: RawResult<CheckoutProcess, SnabbleError>) {
        ...
        case let .success(process):
            updateHeaderView(headerView, with: process.paymentState)
//            continuePolling = updateView(process, result.rawJson)
...
//                self.paymentFinished(false, process, rawJson)
    ...
    private func updateHeaderView(_ headerView: CheckoutHeaderView?, with paymentState: PaymentState) {
        headerView?.configure(with: CheckoutStepStatus.from(paymentState: paymentState))
    }
