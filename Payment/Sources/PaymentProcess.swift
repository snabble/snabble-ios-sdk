...
extension PaymentProcess {
    ...
    private func handleCheckoutResult(_ result: RawResult<CheckoutProcess, SnabbleError>, method: PaymentMethod, completion: @escaping @Sendable (Result<UIViewController, SnabbleError>) -> Void) {
        ...
        case .success(let process):
            Log.info("Checkout process created successfully")
        ...
        case .failure(let error):
            Log.error("Checkout process creation failed: \(error)")
    ...
    private func handleCheckoutProcess(process: CheckoutProcess, completion: @escaping @Sendable (Result<UIViewController, SnabbleError>) -> Void) {
        if let violation = checkViolation(for: process) {
            Log.info("Violation detected, notifying delegate")
        ...
        if let viewController = checkoutVC {
            Log.info("Got checkout view controller: \(String(describing: type(of: viewController)))")
            completion(.success(viewController))
        } else {
            Log.error("Failed to create checkout view controller")
            self.paymentDelegate?.showWarningMessage(Asset.localizedString(forKey: "Snabble.Payment.errorStarting"))
    ...
    static func checkoutViewController(for process: CheckoutProcess,
        ...
        Log.info("checkoutViewController called with paymentState: \(process.paymentState)")

        guard let rawMethod = RawPaymentMethod(rawValue: process.paymentMethod) else {
            Log.error("Invalid payment method: \(process.paymentMethod)")
            return nil
        }

        if process.paymentState == .successful || process.paymentState == .failed {
            // Payment is already finished, notify delegate and return nil
            // The caller should handle navigation to success/failure screen
            Log.info("Payment already finished with state: \(process.paymentState)")
            paymentDelegate?.checkoutFinished(cart, process)
            return nil
        }

        Log.info("Creating checkout display for state: \(process.paymentState)")
