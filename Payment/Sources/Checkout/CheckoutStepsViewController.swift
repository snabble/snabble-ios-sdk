...
struct CheckoutStepRow: View {
    ...
    var body: some View {
        switch step.kind {
        case .default:
            CheckoutStepView(model: step, checkoutModel: model)
        case .information:
            CheckoutInformationView(model: step, checkoutModel: model)
        }
    }
}
        ...
        VStack {
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ForEach(model.checkoutSteps, id: \.self) { step in
                    CheckoutStepRow(step: step, model: model)
                        .padding(10)
...
final class CheckoutStepsViewController: UIHostingController<CheckoutView> {
    ...
    @objc func stepAction(userInfo: [String: Any]?) {
        if let userInfo = userInfo {
            ...
            if userInfo["receiptLink"] is SnabbleCore.Link,
               let orderId = viewModel.checkoutProcess?.orderID {
                let receiptDetailScreen = ReceiptDetailScreen(
                    orderId: orderId,
                    projectId: viewModel.shop.projectId
                )
                let hostingController = UIHostingController(rootView: receiptDetailScreen)
