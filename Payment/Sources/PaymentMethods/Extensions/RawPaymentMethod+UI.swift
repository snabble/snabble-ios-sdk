...
extension RawPaymentMethod {
    ...
    func checkoutDisplayViewController(shop: Shop,
        ...
        case .deDirectDebit,
                ...
                .twint,
                .postFinanceCard:
            guard let process = checkoutProcess else {
                return nil
            }

            // Online payment methods: show checkout steps view with polling
            let checkoutVC = CheckoutStepsViewController(shop: shop, shoppingCart: shoppingCart, checkoutProcess: process)
            checkoutVC.paymentDelegate = delegate
            paymentDisplay = checkoutVC
        case .gatekeeperTerminal:
            // Gatekeeper is handled in checkoutViewController, should not reach here
            return nil
        }
